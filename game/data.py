...
OPERATORS = [
    ...
    Operator(id="leizi", name="惊蛰", tier=1, cost=1, pacts=["yan"],
        traits=[Trait(TraitType.STACK_SINGLE, TriggerType.ON_OBTAIN, "yan", 1, 2, "<获得时>获得等于调度中心等级的【炎】层数")],
    ...
    Operator(id="xiaoman", name="小满", tier=2, cost=2, pacts=["yan", "agility"],
        traits=[Trait(TraitType.STACK_CONTINUOUS, TriggerType.ON_REST_END, "yan", 1, 2, "<休整期结束时>本回合每获1名干员【炎】+1")],
        base_hp=900, base_atk=340, base_def=90),
    Operator(id="shacao", name="莎草", tier=2, cost=2, pacts=["sargon"],
        traits=[Trait(TraitType.STACK_CONTINUOUS, TriggerType.ON_COMBAT_START, "sargon", 5, 10, "<部署时>【萨尔贡】层数+5(精锐+10)")],
    ...
    Operator(id="excu", name="送葬人", tier=2, cost=2, pacts=["laterano", "precision"],
        traits=[Trait(TraitType.STACK_CONTINUOUS, TriggerType.ON_COMBAT_START, "precision", 3, 6, "<战斗开始时>【精准】【拉特兰】层数+3(精锐+6)")],
    ...
    Operator(id="gravel", name="砾", tier=2, cost=2, pacts=["kazimierz", "indomitable"],
        traits=[Trait(TraitType.STACK_CONTINUOUS, TriggerType.ON_COMBAT_START, "kazimierz", 2, 2, "<部署时>【卡西米尔】+2"),
    ...
    Operator(id="philae", name="菲莱", tier=3, cost=3, pacts=["kazimierz", "precision"],
        traits=[Trait(TraitType.STACK_CONTINUOUS, TriggerType.ON_COMBAT_START, "kazimierz", 6, 12, "<部署时>【卡西米尔】【精准】层数+6(精锐+12)")],
    ...
    Operator(id="silverash", name="银灰", tier=4, cost=5, pacts=["kjerag", "investor", "agility"],
        traits=[Trait(TraitType.STACK_CONTINUOUS, TriggerType.ON_COMBAT_START, "kjerag", 1, 2, "<战斗开始时>【谢拉格】层数+3(精锐+6)")],
    ...
    Operator(id="ulpianus", name="乌尔比安", tier=5, cost=8, pacts=["aegir"],
        traits=[Trait(TraitType.STACK_CONTINUOUS, TriggerType.ON_COMBAT_START, "aegir", 3, 6, "<战斗开始时>【阿戈尔】层数+3(精锐+6), 每叠2层攻击力+2%")],
        base_hp=2000, base_atk=650, base_def=180, block_count=2),
    Operator(id="surtr", name="史尔特尔", tier=5, cost=8, pacts=["assault", "arcane"],
        traits=[Trait(TraitType.STACK_CONTINUOUS, TriggerType.ON_COMBAT_START, "assault", 1, 2, "<部署时>【突袭】层数+1(每场至多50/精锐100层)")],
    ...
    Operator(id="skadi_corrupt", name="浊心斯卡蒂", tier=6, cost=12, pacts=["aegir"],
        traits=[Trait(TraitType.STACK_CONTINUOUS, TriggerType.ON_COMBAT_START, "aegir", 5, 10, "<战斗开始时>【阿戈尔】层数+5(精锐+10), 每叠10层HP回复翻倍")],
    ...
    Operator(id="nearl_radiant", name="耀骑士临光", tier=6, cost=12, pacts=["kazimierz", "assault"],
        traits=[Trait(TraitType.STACK_CONTINUOUS, TriggerType.ON_COMBAT_START, "kazimierz", 5, 10, "<战斗开始时>【卡西米尔】层数+5(精锐+10), 再部署-20%")],
    ...
    Operator(id="saint_angel", name="圣约送葬人", tier=6, cost=12, pacts=["laterano", "precision"],
        traits=[Trait(TraitType.STACK_CONTINUOUS, TriggerType.ON_COMBAT_START, "laterano", 1, 2, "<战斗开始时>【拉特兰】【精准】层数+1(精锐+2)")],
        base_hp=1500, base_atk=620, base_def=150, atk_speed=0.9),
    # === 新增71名干员(PRTS数据) ===
    # I阶
    Operator(id="yueyue", name="跃跃", tier=1, cost=1, pacts=["precision"], traits=[Trait(TraitType.STACK_CONTINUOUS, TriggerType.ON_COMBAT_START, "precision", 1, 2, "<部署时>【精准】层数+1")], base_hp=800, base_atk=340, base_def=70, atk_speed=1.0),
    Operator(id="gumi", name="古米", tier=1, cost=1, pacts=["fortification"], traits=[Trait(TraitType.COMBAT, TriggerType.ON_COMBAT_START, "fortification", 1, 2, "<战斗开始时>自身防御+30%")], base_hp=1400, base_atk=220, base_def=140, atk_speed=1.5, block_count=3),
    Operator(id="podengo", name="波登可", tier=1, cost=1, pacts=["assistance"], traits=[Trait(TraitType.STACK_CONTINUOUS, TriggerType.ON_REST_END, "assistance", 1, 2, "<休整期结束时>【助力】层数+1")], base_hp=700, base_atk=300, base_def=60, atk_speed=1.6),
    ...
    Operator(id="indigo", name="深靛", tier=1, cost=1, pacts=["arcane","precision"], traits=[Trait(TraitType.STACK_SINGLE, TriggerType.ON_OBTAIN, "arcane", 2, 4, "<获得时>【奥术】层数+2")], base_hp=800, base_atk=350, base_def=55, atk_speed=1.6),
    ...
    Operator(id="liskarm", name="雷蛇", tier=1, cost=1, pacts=["indomitable"], traits=[Trait(TraitType.STACK_CONTINUOUS, TriggerType.ON_DEFEAT, "indomitable", 2, 4, "<被击倒时>【不屈】层数+2")], base_hp=1300, base_atk=250, base_def=170, atk_speed=1.2, block_count=3),
    # II阶
    Operator(id="silence", name="赫默", tier=2, cost=2, pacts=["foresight"], traits=[Trait(TraitType.PREP, TriggerType.ON_REST_END, "foresight", 2, 4, "<休整期结束时>获得1资金")], base_hp=850, base_atk=370, base_def=80, atk_speed=1.6, healer=True),
    Operator(id="luoluo", name="洛洛", tier=2, cost=2, pacts=["victoria","arcane"], traits=[Trait(TraitType.STACK_CONTINUOUS, TriggerType.ON_REST_END, "victoria", 2, 4, "<休整期结束时>【维多利亚】层数+2")], base_hp=900, base_atk=400, base_def=75, atk_speed=1.6),
    Operator(id="kazemaru", name="风丸", tier=2, cost=2, pacts=["solo", "foresight"], traits=[Trait(TraitType.STACK_SINGLE, TriggerType.ON_COMBAT_START, "solo", 2, 4, "<部署时>【独行】层数+2")], base_hp=950, base_atk=430, base_def=70, atk_speed=1.0),
    Operator(id="tibi", name="蒂比", tier=2, cost=2, pacts=["agility"], traits=[Trait(TraitType.STACK_CONTINUOUS, TriggerType.ON_REST_END, "agility", 2, 4, "<休整期结束时>【灵巧】层数+2")], base_hp=880, base_atk=380, base_def=85, atk_speed=1.3, block_count=1),
    ...
    Operator(id="zheya", name="折桠", tier=2, cost=2, pacts=["fortification","solo"], traits=[Trait(TraitType.STACK_CONTINUOUS, TriggerType.ON_DEFEAT, "fortification", 3, 6, "<被击倒时>【坚守】层数+3")], base_hp=1300, base_atk=300, base_def=150, atk_speed=1.5, block_count=2),
    Operator(id="huihao", name="灰毫", tier=2, cost=2, pacts=["kazimierz","fortification"], traits=[Trait(TraitType.STACK_SINGLE, TriggerType.ON_COMBAT_START, "kazimierz", 3, 6, "<部署时>【卡西米尔】层数+3")], base_hp=1400, base_atk=280, base_def=160, atk_speed=1.5, block_count=3),
    ...
    # III阶
    Operator(id="duanya", name="断崖", tier=3, cost=3, pacts=["agility"], traits=[Trait(TraitType.STACK_CONTINUOUS, TriggerType.ON_COMBAT_START, "agility", 3, 6, "<部署时>【灵巧】层数+3")], base_hp=1200, base_atk=480, base_def=130, atk_speed=1.2, block_count=2),
    ...
    Operator(id="xuelie", name="雪猎", tier=3, cost=3, pacts=["kjerag","precision"], traits=[Trait(TraitType.STACK_SINGLE, TriggerType.ON_COMBAT_START, "kjerag", 4, 8, "<部署时>【谢拉格】层数+4(单次)")], base_hp=1100, base_atk=470, base_def=110, atk_speed=1.0, block_count=1),
    Operator(id="xiaguang", name="瑕光", tier=3, cost=3, pacts=["kazimierz","assault"], traits=[Trait(TraitType.STACK_CONTINUOUS, TriggerType.ON_REST_END, "kazimierz", 4, 8, "<休整期结束时>【卡西米尔】层数+4")], base_hp=1300, base_atk=440, base_def=160, atk_speed=1.5, block_count=2),
    ...
    Operator(id="rendong", name="忍冬", tier=3, cost=3, pacts=["siracusa","agility"], traits=[Trait(TraitType.STACK_CONTINUOUS, TriggerType.ON_REST_START, "siracusa", 3, 6, "<进入休整期时>【叙拉古】层数+3")], base_hp=1200, base_atk=480, base_def=130, atk_speed=1.2, block_count=2),
    Operator(id="siye", name="伺夜", tier=3, cost=3, pacts=["siracusa", "foresight"], traits=[Trait(TraitType.STACK_CONTINUOUS, TriggerType.ON_COMBAT_START, "siracusa", 4, 8, "<战斗开始时>【叙拉古】层数+4")], base_hp=1000, base_atk=460, base_def=85, atk_speed=1.0),
    Operator(id="yela", name="耶拉", tier=3, cost=3, pacts=["kjerag","assistance"], traits=[Trait(TraitType.STACK_CONTINUOUS, TriggerType.ON_REST_END, "kjerag", 3, 6, "<休整期结束时>【谢拉格】层数+3")], base_hp=1100, base_atk=450, base_def=100, atk_speed=1.6),
    ...
    # IV阶
    Operator(id="xinyangjiaobanji", name="信仰搅拌机", tier=4, cost=5, pacts=["laterano","fortification"], traits=[Trait(TraitType.STACK_CONTINUOUS, TriggerType.ON_COMBAT_START, "laterano", 4, 8, "<部署时>【拉特兰】层数+4")], base_hp=1500, base_atk=500, base_def=180, atk_speed=1.5, block_count=3),
    Operator(id="mostima", name="莫斯提马", tier=4, cost=5, pacts=["laterano","arcane"], traits=[Trait(TraitType.STACK_CONTINUOUS, TriggerType.ON_REST_END, "arcane", 4, 8, "<休整期结束时>【奥术】层数+4")], base_hp=1200, base_atk=580, base_def=100, atk_speed=1.6),
    Operator(id="yineisi", name="伊内丝", tier=4, cost=5, pacts=["foresight","assault"], traits=[Trait(TraitType.STACK_SINGLE, TriggerType.ON_COMBAT_START, "foresight", 6, 12, "<部署时>【远见】层数+6(单次)")], base_hp=1400, base_atk=520, base_def=140, atk_speed=1.0, block_count=2),
    Operator(id="hanmangkeluosi", name="寒芒克洛丝", tier=4, cost=5, pacts=["precision"], traits=[Trait(TraitType.COMBAT, TriggerType.ON_COMBAT_START, "", 3, 6, "<战斗开始时>攻速+60, 对低血量敌人伤害+30%")], base_hp=1100, base_atk=560, base_def=90, atk_speed=0.9),
    Operator(id="shuiyue", name="水月", tier=4, cost=5, pacts=["aegir","solo"], traits=[Trait(TraitType.STACK_CONTINUOUS, TriggerType.ON_DEFEAT, "aegir", 5, 10, "<被击倒时>【阿戈尔】层数+5")], base_hp=1300, base_atk=550, base_def=120, atk_speed=1.2, block_count=1),
    Operator(id="aluoma", name="阿罗玛", tier=4, cost=5, pacts=["siracusa","arcane"], traits=[Trait(TraitType.STACK_CONTINUOUS, TriggerType.ON_REST_START, "siracusa", 4, 8, "<进入休整期时>【叙拉古】层数+4")], base_hp=1200, base_atk=540, base_def=110, atk_speed=1.5),
    ...
    Operator(id="laienhate", name="莱恩哈特", tier=4, cost=5, pacts=["agility","precision"], traits=[Trait(TraitType.STACK_CONTINUOUS, TriggerType.ON_COMBAT_START, "agility", 4, 8, "<部署时>【灵巧】层数+4")], base_hp=1150, base_atk=530, base_def=100, atk_speed=1.3),
    ...
    Operator(id="yanwei", name="焰尾", tier=4, cost=5, pacts=["kazimierz"], traits=[Trait(TraitType.STACK_CONTINUOUS, TriggerType.ON_COMBAT_START, "kazimierz", 4, 8, "<部署时>【卡西米尔】层数+4")], base_hp=1300, base_atk=520, base_def=150, atk_speed=1.0, block_count=1),
    Operator(id="yuanya", name="远牙", tier=4, cost=5, pacts=["kazimierz","precision"], traits=[Trait(TraitType.STACK_CONTINUOUS, TriggerType.ON_COMBAT_START, "kazimierz", 5, 10, "<战斗开始时>【卡西米尔】层数+5")], base_hp=1000, base_atk=600, base_def=80, atk_speed=1.0),
    ...
    Operator(id="kanielian", name="卡涅利安", tier=4, cost=5, pacts=["sargon","assistance"], traits=[Trait(TraitType.STACK_CONTINUOUS, TriggerType.ON_REST_END, "sargon", 4, 8, "<休整期结束时>【萨尔贡】层数+4")], base_hp=1300, base_atk=520, base_def=140, atk_speed=1.6),
    Operator(id="mowang", name="魔王", tier=4, cost=5, pacts=["harmony"], traits=[Trait(TraitType.STACK_CONTINUOUS, TriggerType.ON_REST_END, "harmony", 4, 8, "<休整期结束时>【调和】层数+4")], base_hp=1400, base_atk=500, base_def=130, atk_speed=1.5),
    ...
    # V阶
    Operator(id="nifu", name="妮芙", tier=5, cost=8, pacts=["agility"], traits=[Trait(TraitType.STACK_CONTINUOUS, TriggerType.ON_REST_END, "agility", 5, 10, "<休整期结束时>【灵巧】层数+5")], base_hp=1200, base_atk=620, base_def=95, atk_speed=1.0),
    Operator(id="titi", name="缇缇", tier=5, cost=8, pacts=["sargon","precision"], traits=[Trait(TraitType.STACK_SINGLE, TriggerType.ON_COMBAT_START, "sargon", 8, 16, "<部署时>【萨尔贡】层数+8(单次)")], base_hp=1400, base_atk=600, base_def=140, atk_speed=1.2, block_count=1),
    Operator(id="zhuhuang", name="烛煌", tier=5, cost=8, pacts=["victoria","yan"], traits=[Trait(TraitType.COMBAT, TriggerType.ON_COMBAT_START, "", 5, 10, "<战斗开始时>攻击+25%, 对自己周围敌人造成灼烧伤害")], base_hp=1500, base_atk=650, base_def=170, atk_speed=1.5, block_count=2),
    Operator(id="yindelaixi", name="隐德来希", tier=5, cost=8, pacts=["indomitable","agility"], traits=[Trait(TraitType.STACK_CONTINUOUS, TriggerType.ON_DEFEAT, "indomitable", 6, 12, "<被击倒时>【不屈】层数+6")], base_hp=1600, base_atk=580, base_def=150, atk_speed=1.2, block_count=2),
    ...
    Operator(id="xi", name="夕", tier=5, cost=8, pacts=["yan","arcane"], traits=[Trait(TraitType.STACK_CONTINUOUS, TriggerType.ON_COMBAT_START, "yan", 5, 10, "<部署时>【炎】层数+5")], base_hp=1300, base_atk=650, base_def=110, atk_speed=1.6),
    Operator(id="guimingyouling", name="归溟幽灵鲨", tier=5, cost=8, pacts=["aegir","indomitable"], traits=[Trait(TraitType.STACK_CONTINUOUS, TriggerType.ON_DEFEAT, "aegir", 6, 12, "<被击倒时>【阿戈尔】层数+6")], base_hp=1800, base_atk=600, base_def=160, atk_speed=1.2, block_count=2),
    Operator(id="linyuyinhui", name="凛御银灰", tier=5, cost=8, pacts=["kjerag","investor","agility"], traits=[Trait(TraitType.STACK_SINGLE, TriggerType.ON_COMBAT_START, "kjerag", 8, 16, "<部署时>【谢拉格】层数+8(单次)")], base_hp=1600, base_atk=620, base_def=170, atk_speed=1.2, block_count=2),
    Operator(id="yinxingjici", name="引星棘刺", tier=5, cost=8, pacts=["agility", "foresight"], traits=[Trait(TraitType.COMBAT, TriggerType.ON_COMBAT_START, "", 5, 10, "<战斗开始时>攻速+80, 再部署时间-50%")], base_hp=1400, base_atk=640, base_def=130, atk_speed=1.0, block_count=2),
    Operator(id="shan", name="山", tier=5, cost=8, pacts=["investor"], traits=[Trait(TraitType.STACK_CONTINUOUS, TriggerType.ON_REST_END, "investor", 5, 10, "<休整期结束时>【投资人】层数+5")], base_hp=1800, base_atk=580, base_def=180, atk_speed=1.0, block_count=2),
    Operator(id="anjielinna", name="安洁莉娜", tier=5, cost=8, pacts=["siracusa", "foresight"], traits=[Trait(TraitType.STACK_CONTINUOUS, TriggerType.ON_REST_START, "siracusa", 5, 10, "<进入休整期时>【叙拉古】层数+5")], base_hp=1000, base_atk=560, base_def=85, atk_speed=1.6),
    ...
    Operator(id="luwuguan", name="录武官", tier=5, cost=8, pacts=["yan", "foresight"], traits=[Trait(TraitType.STACK_SINGLE, TriggerType.ON_OBTAIN, "yan", 6, 12, "<获得时>【炎】层数+6(单次)")], base_hp=1500, base_atk=580, base_def=160, atk_speed=1.5, block_count=2, healer=True),
    # VI阶
    Operator(id="leimuan", name="蕾缪安", tier=6, cost=12, pacts=["laterano","precision"], traits=[Trait(TraitType.STACK_CONTINUOUS, TriggerType.ON_COMBAT_START, "laterano", 6, 12, "<部署时>【拉特兰】层数+6")], base_hp=1600, base_atk=680, base_def=150, atk_speed=0.9),
    Operator(id="shenglingchuxue", name="圣聆初雪", tier=6, cost=12, pacts=["kjerag","arcane"], traits=[Trait(TraitType.STACK_CONTINUOUS, TriggerType.ON_REST_END, "kjerag", 6, 12, "<休整期结束时>【谢拉格】层数+6")], base_hp=1500, base_atk=650, base_def=140, atk_speed=1.5),
    ...
    Operator(id="suxin", name="塑心", tier=6, cost=12, pacts=["laterano"], traits=[Trait(TraitType.STACK_CONTINUOUS, TriggerType.ON_REST_END, "laterano", 6, 12, "<休整期结束时>【拉特兰】层数+6")], base_hp=1400, base_atk=700, base_def=130, atk_speed=1.5),
    Operator(id="midiexiang", name="迷迭香", tier=6, cost=12, pacts=["precision","foresight","harmony"], traits=[Trait(TraitType.STACK_SINGLE, TriggerType.ON_COMBAT_START, "precision", 10, 20, "<部署时>【精准】层数+10(单次)")], base_hp=1500, base_atk=720, base_def=140, atk_speed=1.0),
    Operator(id="xinyuenengtianshi", name="新约能天使", tier=6, cost=12, pacts=["laterano"], traits=[Trait(TraitType.COMBAT, TriggerType.ON_COMBAT_START, "", 6, 12, "<战斗开始时>攻速+100, 连续攻击伤害递增")], base_hp=1300, base_atk=750, base_def=100, atk_speed=0.85),
    Operator(id="liuming", name="流明", tier=6, cost=12, pacts=["assistance"], traits=[Trait(TraitType.PREP, TriggerType.ON_REST_END, "", 0, 0, "<休整期结束时>全队回复20%最大生命")], base_hp=1400, base_atk=550, base_def=120, atk_speed=1.6),
    Operator(id="qiubai", name="仇白", tier=6, cost=12, pacts=["yan","assault"], traits=[Trait(TraitType.STACK_CONTINUOUS, TriggerType.ON_COMBAT_START, "yan", 6, 12, "<部署时>【炎】层数+6")], base_hp=1900, base_atk=700, base_def=190, atk_speed=1.2, block_count=2),
    Operator(id="suguangxingyuan", name="溯光星源", tier=6, cost=12, pacts=["arcane","agility"], traits=[Trait(TraitType.STACK_CONTINUOUS, TriggerType.ON_REST_START, "arcane", 6, 12, "<进入休整期时>【奥术】层数+6")], base_hp=1400, base_atk=680, base_def=120, atk_speed=1.6),
    Operator(id="huangwulapulande", name="荒芜拉普兰德", tier=6, cost=12, pacts=["siracusa"], traits=[Trait(TraitType.COMBAT, TriggerType.ON_COMBAT_START, "", 6, 12, "<战斗开始时>攻击+35%, 对精英和领袖伤害翻倍")], base_hp=1700, base_atk=720, base_def=160, atk_speed=1.2, block_count=2),
    Operator(id="jian", name="锏", tier=6, cost=12, pacts=["kjerag","kazimierz","agility"], traits=[Trait(TraitType.STACK_SINGLE, TriggerType.ON_COMBAT_START, "kjerag", 10, 20, "<部署时>【谢拉格】层数+10(单次)")], base_hp=1800, base_atk=700, base_def=180, atk_speed=1.0, block_count=2),
...
ENEMIES = {
    ...
    "boss": Enemy(id="boss", name="领袖·整合之影", hp=15000, max_hp=15000, atk=500, defense=300, speed=0.6, is_elite=True, is_boss=True),
}
